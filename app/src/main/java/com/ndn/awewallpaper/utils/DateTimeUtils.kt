...
import com.ndn.awewallpaper.utils.extension.convertDateToDate
import com.ndn.awewallpaper.utils.extension.convertDateToString
import com.ndn.awewallpaper.utils.extension.convertUiFormatToDataFormat
...
object DateTimeUtils {
    ...
    fun getDateStringFromDatePicker(year: Int, month: Int, dayOfMonth: Int,
    ...
    fun getCurrentTimeSearchCourse(): String? {
        val cal: Calendar = getCalendarActualForSearch()
        val minute = if(cal.get(Calendar.MINUTE) < 10) "0${cal.get(Calendar.MINUTE)}" else cal.get(Calendar.MINUTE).toString()
        return cal.get(Calendar.HOUR_OF_DAY).toString() + ":" + minute
    }

    fun getCalendarActualForSearch(): Calendar {
        val cal: Calendar = Calendar.getInstance()
        cal.add(Calendar.MINUTE, Constants.MINIMUM_MINUTE_DURATION)
    ...
    fun convertDateTimeWithFormat(dateTime: String, fromFormat: String, toFormat: String,
    ...
    fun twoDaysEqual(firstDay: Date?, secondDay: Date?): Boolean {
        return firstDay?.convertDateToDate(
                outputFormat = DATE_TIME_FORMAT_YYYY_MM_DD) == secondDay?.convertDateToDate(
    ...
    fun replaceHHmm(date: String?, hour: String?, locale: Locale = Locale.JAPAN): String {
        ...
        val newDate = date.convertUiFormatToDataFormat(
                outputFormat = DateTimeUtils.DATE_TIME_FORMAT_YYYY_MM_DD_EN,
                locale = locale)
        val newDateTime = "$newDate $hour"
        return newDateTime.convertUiFormatToDataFormat(
                inputFormat = DateTimeUtils.DATE_TIME_FORMAT_YYYY_MM_DD_HH_MM,
                outputFormat = DateTimeUtils.DATE_TIME_FORMAT_UTC,
